        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={i} className="top5-item">
              <span className={`top5-rank${i === 0 ? " gold" : ""}`}>
                {i + 1}
              </span>
...
export default function DashboardSection() {
  ...
  const handleFavSelect = async (e) => {
    ...
    try {
      ...
      // 시간대별 유동인구 순위 (카드5 뒷면)
      const timeRanking = TIME_SLOTS.map((s) => ({ name: s.range, val: s.val }))
        .sort((a, b) => b.val - a.val)
        .slice(0, 5);
      ...
      const salesTop5 =
        topIndustries?.salesTop5?.map((it) => ({
          name: it.industryName,
          val: it.monthlySalesAmount,
        })) ?? [];
      const storeTop5 =
        topIndustries?.storeTop5?.map((it) => ({
          name: it.industryName,
          val: it.storeCount,
        })) ?? [];
  ...
  const storeCountVal = reportLoading ? null : (reportData?.storeCount ?? null);

  const dailyPopVal = reportLoading ? null : (reportData?.dailyPop ?? null);

  const peakSlot = reportLoading ? null : (reportData?.peakSlot ?? null);
  ...
  const topIndustrySalesVal =
    salesTop5[0]?.val != null ? Math.round(salesTop5[0].val / 10000) : null;
      ...
      <div className="dashboard-inner">
        <div className="dashboard-section-header" data-reveal data-delay="0">
          ...
          <p className="dashboard-section-desc">
            즐겨찾기 상권을 선택하면 매출·유동인구·경쟁 현황을 한눈에 확인할 수
            있습니다.
          </p>
        </div>
        ...
        <div className="hero-dashboard-grid">
          ...
          >
            <div
              className={`card-flip-inner${flippedCards.revenue ? " flipped" : ""}`}
            >
              <div className="card-front">
                ...
                <div className="gc-illus">
                  <MdPointOfSale
                    className="gc-illus-icon gc-illus-icon--revenue"
                    aria-hidden="true"
                  />
                </div>
                <div className="gc-val-wrap">
                  <div className="gc-val-main">
                    ...
                    {salesVal != null ? (
                      <>
                        <strong>{salesVal.toLocaleString()}</strong>원
                      </>
                    ) : (
                      <ValSkeleton />
                    )}
          ...
          >
            <div
              className={`card-flip-inner${flippedCards.store ? " flipped" : ""}`}
            >
              <div className="card-front">
                <CardHeader
                  title="업소 수는?"
                ...
                <div className="gc-illus">
                  <MdStorefront
                    className="gc-illus-icon gc-illus-icon--store"
                    aria-hidden="true"
                  />
                </div>
                <div className="gc-val-wrap">
          ...
          >
            <div
              className={`card-flip-inner${flippedCards.pop ? " flipped" : ""}`}
            >
              <div className="card-front">
                <CardHeader
                  title="유동인구는?"
                ...
                <div className="gc-illus">
                  <MdGroups
                    className="gc-illus-icon gc-illus-icon--population"
                    aria-hidden="true"
                  />
                </div>
                <div className="gc-val-wrap">
              ...
              <CardBack
                ...
                items={ageRanking}
                formatVal={(v) =>
                  `${Math.round(v / 10000).toLocaleString()}만명`
                }
              />
            </div>
          </div>
          ...
          >
            <div
              className={`card-flip-inner${flippedCards.catSales ? " flipped" : ""}`}
            >
              <div className="card-front">
                <CardHeader
                  title="주요 업종 매출은?"
                ...
                <div className="gc-illus">
                  <MdBusinessCenter
                    className="gc-illus-icon gc-illus-icon--category"
                    aria-hidden="true"
                  />
                </div>
                <div className="gc-val-wrap">
                  <div className="gc-val-main">
                    ...
                    {topIndustrySalesVal != null ? (
                      <>
                        <strong>{topIndustrySalesVal.toLocaleString()}</strong>
                        원
                      </>
                    ) : reportLoading ? (
                      <ValSkeleton />
          ...
          >
            <div
              className={`card-flip-inner${flippedCards.peak ? " flipped" : ""}`}
            >
              <div className="card-front">
                <CardHeader
                  title="피크 시간대는?"
                ...
                <div className="gc-illus">
                  <MdAccessTimeFilled
                    className="gc-illus-icon gc-illus-icon--peak"
                    aria-hidden="true"
                  />
                </div>
                <div className="gc-val-wrap">
